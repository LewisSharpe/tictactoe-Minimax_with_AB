...
namespace Minimax
{
    ...
    class AIPlayer : Player
    {
        ...
        // GET MOVE
        public Tuple<int, int> PlaceInTwoNeighbour(counters us, GameBoard<counters> board, GameBoard<int> scoreBoard)
        {
            for (int x = 1; x <= 7; x++)
                for (int y = 1; y <= 7; y++)
                {
                    ...
                    for (int xx = -1; xx <= 7; xx++)
                        for (int yy = -1; yy <= 7; yy++)
                        {
                            ...
                            if (board[x, y] == us && board[x, y] == board[x + xx, y + yy])
                                continue;
                            else if (board[x - 1, y] == counters.EMPTY)
                                return new Tuple<int, int>(x - 1, y);
                            else if (board[x + 1, y] == counters.EMPTY)
                                return new Tuple<int, int>(x + 1, y);
                            else if (board[x, y + 1] == counters.EMPTY)
            ...
            else if (board.IsMiddleEmpty() == true || board.IsBottomLeftEmpty() == true || board.IsBottomRightEmpty() == true || board.IsTopLeftEmpty() == true || board.IsTopRightEmpty() == true)
            {
                ...
                for (int i = 0; i < availableMoves.Count; i++)
                {
                   ...
                   // }
                    ...
                    "depth level: " + ply + Environment.NewLine +
                    "elapsed time for move: " + stopwatch.Elapsed + "; " +
                    "no. of remaining moves left: " + availableMoves.Count + "; " +
                    "best score: " + bestScore + "; " +
                    "best move: " + bestMove.Item1 + "," + bestMove.Item2 + Environment.NewLine +
                                ...
                                if (neigh.Item2 < 0 & neigh.Item2 > 7)
                                {
                                    if (copy[neigh.Item1, neigh.Item2] == counters.EMPTY)
                                    {
                                        ...
                                        if (copy[neigh.Item1, neigh.Item2] == counter)
                                        {
                                            ...
                                            scoreBoard[neigh.Item1, neigh.Item2] = score;
                           //               Console.ReadLine();
                                            return new Tuple<int, Tuple<int, int>, GameBoard<counters>, GameBoard<int>>(score, PlaceInTwoNeighbour(us,board,scoreBoard), board, scoreBoard); // return return
                                        }
                                    }
                                }
                            }
