...
*/
    ...
    /*
public Tuple<GameBoard<counters>, GameBoard<int>, int, int, int, bool, int> AlphaBeta(GameBoard<counters> board, GameBoard<int> scoreBoard, int ply, int alpha, int beta, bool mmax, int score)
{
    // variable declarations
    List<Tuple<int, int>> availableMoves = getAvailableMoves(board, positions);
    int bestScore = mmax ? -1001 : 1001;
    score = Consts.MIN_SCORE; // current score of move
    Tuple<int, int> Move = new Tuple<int, int>(0, 0);
    Tuple<int, int> bestMove = new Tuple<int, int>(1, 1);  // best move with score// THRESHOLD <=============
    alpha = Consts.MIN_SCORE;
    beta = Consts.MAX_SCORE;
    if (Move.Item1 < bestScore && ply == 0)
    {
        return new Tuple<GameBoard<counters>, GameBoard<int>, int, int, int, bool, int>(board, scoreBoard, ply, alpha, beta, mmax, score);
    }
    else if (mmax)
    {
        bestScore = -9999;
        ...
        {
            Tuple<int, Tuple<int, int>, GameBoard<counters>, GameBoard<int>> mm_result = Minimax(copy, Flip(counter), ply + 1, Move, !mmax, scoreBoard, ref cont);
            bestScore = beta;
            alpha = max(alpha, bestScore);                                                                                                                                                         // alpha = max(alpha, bestVal)
            if (beta <= alpha)
            {
                break;
            }
            return new Tuple<GameBoard<counters>, GameBoard<int>, int, int, int, bool, int>(board, scoreBoard, ply, alpha, beta, mmax, bestScore);
        } // end foreach
    }
    else if (!mmax)
    {
        ...
        foreach (Tuple<int, int> move in availableMoves)
        {
            Tuple<int, Tuple<int, int>, GameBoard<counters>, GameBoard<int>> mm_result =
                Minimax(copy, Flip(counter), ply + 1, Move, !mmax, scoreBoard, ref cont);
            bestScore = alpha;
