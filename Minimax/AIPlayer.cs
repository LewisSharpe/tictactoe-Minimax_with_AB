...
namespace Minimax
{
        ...
        where T : IComparable
    {
        ...
        // GET MOVE
        public override Tuple<int, int> GetMove(GameBoard<counters> board)
        {
            ...
            Tuple<int, Tuple<int, int>, GameBoard<counters>> result;
     ...
     //       Debug.Assert(us == counters.NOUGHTS || us == counters.CROSSES);
       ...
       //     Debug.Assert(us == counters.NOUGHTS || us == counters.CROSSES);
        ...
        public int EvalCurrentBoard(GameBoard<counters> board, int ourindex, counters us)
        {
            // score decs
            int score = 10;
            int two_score = 10;
        ...
        // MINIMAX FUNCTION
        public Tuple<int, Tuple<int, int>, GameBoard<counters>> Minimax(GameBoard<counters> board, counters counter, int ply, Tuple<int, int> positions, bool max, ref int contt)
        {
            // decs
            ...
            if (Win(board, counter))
                return new Tuple<int, Tuple<int, int>, GameBoard<counters>>(1000, positions, board);
            else if (Win(board, this.otherCounter))
                return new Tuple<int, Tuple<int, int>, GameBoard<counters>>(-1000, positions, board);
            else if (availableMoves.Count == 0)
                return new Tuple<int, Tuple<int, int>, GameBoard<counters>>(0, positions, board);
            // CHECK DEPTH
            else if (ply > maxPly)
            {
                score = EvalCurrentBoard(board, ourindex, us); // call stat evaluation func - takes board and player and gives score to that player
                return new Tuple<int, Tuple<int, int>, GameBoard<counters>>(score, positions, board);
            }
            else if (ply > 0)
            {
                   ...
                   // Debug.Assert(us == counters.NOUGHTS || us == counters.CROSSES);
                    ...
                    if (copy.IsMiddleEmpty() == true)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                cont = 1;
                                return new Tuple<int, Tuple<int, int>, GameBoard<counters>>(score, nodelist[i], board); // return
                            }
                    ...
                    else
                    {
                        ...
                        Tuple<int, Tuple<int, int>, GameBoard<int>> result = Minimax(copy, Flip(counter), ply + 1, Move, max, ref cont);  /* swap player */  // RECURSIVE call  
                            ...
                            if (neigh.Item1 < 0 & neigh.Item1 > 7)
                            {
                                if (neigh.Item2 < 0 & neigh.Item2 > 7)
                                {
                                    if (copy[neigh.Item1, neigh.Item2] == counters.EMPTY)
                                    {
                                        ...
                                        {
                                            score = 100;
                                            return new Tuple<int, Tuple<int, int>, GameBoard<counters>>(score, positions, board); // return
                                        }
                                    }
                                }
                            }
            ...
            return new Tuple<int, Tuple<int, int>, GameBoard<counters>>(bestScore, bestMove, board); // return
        }
    }
}
