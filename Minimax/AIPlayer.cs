...
namespace Minimax
{
    class AIPlayer : Player
    {
        ...
        public override Tuple<int, int> GetMove(GameBoard board)
        {
            ...
            result = Minimax(board, copy, counter, ply, new Tuple<int, int>(0, 0));
            Console.WriteLine("score:" + result.Item1 + "depth:" + maxPly);
            Console.ReadLine();
            return result.Item2;
        }
        ...
        public bool FindOneInARow(GameBoard board, counters us)
        {
            int DirIndex = 0;
            int Dir = 0;
            int oneCount = 1;
            ...
            for (DirIndex = 0; DirIndex < Consts.NO_OF_DIRS; ++DirIndex)
            {
                ...
                oneCount += GetNumForDir(Dir, Dir, board, us);
                oneCount += GetNumForDir(Dir * -1, Dir * -1, board, us);
                if (oneCount == 1)
                {
                    return true;
                }
        ...
        public int EvalCurrentBoard(GameBoard board, int ourindex, counters us) {
          ...
          // two score
            ...
            // one score
            if (FindOneInARow(board, us)) // player win?
                one_score = 10; // player win confirmed
            if (FindOneInARow(board, us + 1)) // opponent win?
            ...
            if (two_score != 0)
            {
              /*   board.DisplayBoard();
                Console.Write("two: " + two_score);
                 ...
                 Console.WriteLine(Consts.MAX_SCORE / 10);
        ...
        // MINIMAX FUNCTION
        public Tuple<int, Tuple<int, int>, GameBoard> Minimax(GameBoard board, GameBoard copy, counters counter, int ply, Tuple<int, int> positions)
        {
            // decs
            counters us = counters.NOUGHTS;
            int ourindex = 1;
            List<Tuple<int, int>> availableMoves = getAvailableMoves(board);
            int bestScore = Consts.MAX_SCORE;
            ...
            else // else run Minimax
            {
                for (int i = 0; i < availableMoves.Count; i++)
                {
                    ...
                    copy = board.Clone(); // board is clone
                    copy[1,4] = counters.NOUGHTS; // places nought in the copy at position 1,4
                    Tuple<int, Tuple<int, int>, GameBoard> result = Minimax(board, copy, Flip(counter), ply + 1, Move);  /* swap player */  // RECURSIVE call // run Minimax                   
                    // print board and score here - is it right? - if ply 0.. -1000 example
                    copy[Move.Item1, Move.Item2] = counter; // updates board sequentially - places count at position ?,?
                    //Console.WriteLine(score);
                    // copy.DisplayBoard(); // display the copy with new counter in place

                    /* NEXT STEPS (22.03.2019):
                     1: replace the contents of the Tuple 'board' with the contents of the
                     Tuple 'copy' with new move placed, using Tuple.Create
                     immutable
                    ...
                    2: start search for next move - repeat above */
