    ...
    public Tuple<int,Tuple<int,int>> Minimax(GameBoard board, counters counter, int ply, Tuple<int,int> positions)
        {
            ...
            Tuple<int, int> Move;
            int bestMove = -1; // best move with score// THRESHOLD <=============
            if (Win(board, counter))
                ...
                return new Tuple<int, Tuple<int, int>>(0, positions); // call stat evaluation func - takes board and player and gives score to that player
            List<int> moves = new List<int>();
            for (int i = 0; i < availableMoves.Count; i++)
            {
                // thresholding

                FindTwoInARow(board, ourindex, us) {
