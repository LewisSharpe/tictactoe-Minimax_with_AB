	    ...
	    */
	    ...
	    Console.WriteLine("__ HWL: OVERALL best result on board {0} and player {1}: {2}", Game_TPL.cntr, Flip(counter), res.ToString());
            while (!Win(board, Flip(counter))) {         
                res = ParSearchWork(board, Flip(counter), ply, positions, true, scoreBoard, stride, 0, bestRes, 1, unconsideredMoves);
                    board[res.Item2.Item1, res.Item2.Item2] = Flip(counter);
                    board.DisplayBoard();
            }
